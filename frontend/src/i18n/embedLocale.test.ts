...
test('supported embed locales include zh-CN, en-US, ko-KR, ja-JP, ru-RU, vi-VN', () => {
...
test('normalizeEmbedLocale maps tags accurately with fallback to vi-VN', () => {
  ...
  assert.equal(normalizeEmbedLocale('vi-VN'), 'vi-VN')
  assert.equal(normalizeEmbedLocale('vi'), 'vi-VN')
  assert.equal(normalizeEmbedLocale('unknown-locale'), 'vi-VN')
  assert.equal(normalizeEmbedLocale('   '), 'vi-VN')
})

...
func TestResolveLanguage(t *testing.T) {
	...
	tests := []struct {
		...
		{"no locale and no context falls back to default", context.Background(), "", "", fallbackDefaultLanguage},
		{"empty context value falls back to default", context.WithValue(
			context.Background(), LanguageContextKey, ""), "", "", fallbackDefaultLanguage},
...
func TestLanguageFromContextOrDefault(t *testing.T) {
	if got := LanguageFromContextOrDefault(context.Background()); got != fallbackDefaultLanguage {
		t.Errorf("LanguageFromContextOrDefault(empty) = %q, want %q", got, fallbackDefaultLanguage)
	}

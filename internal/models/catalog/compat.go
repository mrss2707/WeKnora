...
func DefaultAnthropicMessages() AnthropicMessagesSettings {
	return AnthropicMessagesSettings{
		...
		// DefaultMaxTokens raised from the api.anthropic.com baseline (4096) so
		// completions are not truncated by default; override per-vendor via
		// AnthropicMessagesCompat.DefaultMaxTokens where a lower cap is needed.

...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	must(container.Provide(service.NewModelService, dig.Name("modelServiceBase")))
	must(container.Provide(service.NewModelServiceWithFallback))
	...
	// handler are wired in memory_v2.go behind this single stable call. Must
	// come after the chatpipeline block above: it Invokes immediately to
	// register the memory plugin on *chatpipeline.EventManager, which does
	// not exist yet any earlier.
	...
	must(container.Provide(handler.NewModelPreferenceHandler))

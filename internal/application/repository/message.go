...
//
// Select("*") forces GORM to write zero-value fields too (a plain Updates
// call silently skips them, which is how an assistant message truncated to
// "" or marked incomplete=false failed to persist — see the fix in
// 8b97f164). context_checkpoint and sandbox_checkpoint are excluded: they
// are written only by their own dedicated setters
// (UpdateMessageContextCheckpoint, RewriteSandboxCheckpoints), and most
// UpdateMessage callers load a message from before one of those was set, so
// a blanket zero-value write here would silently erase it — exactly the
// regression TestUpdateMessageKeepsAnExistingContextCheckpoint guards.
func (r *messageRepository) UpdateMessage(ctx context.Context, message *types.Message) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		).Select("*").Omit("context_checkpoint", "sandbox_checkpoint").Updates(message).Error; err != nil {
			return err
		}

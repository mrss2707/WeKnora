...
- **macOS（OrbStack）**：macOS 上可选用 [OrbStack](https://orbstack.dev) 作为 Docker 运行时（`docker context use orbstack` 切换上下文），其余步骤与下文一致；容器内访问宿主机 Ollama 的 `host.docker.internal:11434` 同样可用，无需额外配置。
